...
namespace VFXEpoch
{
	...
	public:
		...
		void setData(T _data, int i, int j) {
			if (i > (m_yCell - 1) || j > (m_xCell - 1) || i < 0 || j < 0)
				assert(i <= (m_yCell - 1) && j <= (m_xCell - 1));
			else
				data[i * m_xCell + j] = _data;
		}
		...
		{
			data[IDX2D(0, 0)] = (data[IDX2D(0, 1)] + data[IDX2D(1, 0)]) * 0.5;
			data[IDX2D(0, m_xCell - 1)] = (data[IDX2D(0, m_xCell - 2)] + data[IDX2D(1, m_xCell - 1)]) * 0.5;
			data[IDX2D(m_yCell - 1, 0)] = (data[IDX2D(m_yCell - 2, 0)] + data[IDX2D(m_yCell - 1, 1)]) * 0.5;
			data[IDX2D(m_yCell - 1, m_xCell - 1)] = (data[IDX2D(m_yCell - 2, m_xCell - 1)] + data[IDX2D(m_yCell - 1, m_xCell - 2)]) * 0.5;
		}
