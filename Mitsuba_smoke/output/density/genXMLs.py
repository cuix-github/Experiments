...
for i in range(0, total_frames):
    ...
    scene = etree.Element("scene", version="0.4.0")
    ...
    # Medium
    medium = etree.SubElement(scene, "medium", type="heterogenous", id="smoke")
    method = etree.SubElement(medium, "string", name="method", value="woodcokc")
    density = etree.SubElement(medium, "volume", name="density", type="gridvolume")
    etree.SubElement(density, "string", name="filename", value="density-%d.vol" % i)
    ...
    sensor_transform = etree.SubElement(sensor, "transofrm", name="toWorld")
    ...
    etree.SubElement(sensor_sampler, "integer", name="sampleCount", value="64")
    ...
    with open("frame_%d.xml" % i, "w") as frame_xml:
