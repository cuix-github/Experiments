...
void exportDensity_uncompressed(int i);
void exportDensity(int i);
void exportDensityABC();
...
void renderFunc() {
	...
	if (running) {
		...
		if(frameCounter == endFrame) running = false;
		glutPostRedisplay();
	}
}
...
void exportDensity_uncompressed(int i){
	char fname[128];
	snprintf(fname, sizeof(fname), "output/density/density-%04i.vol", i);
...
int main(int argc, char **argv) {
	...
	glutCreateWindow("SL3D Solver [density]");
